...
gct_file <- "~/GeneExpressionAnalysis/Data/Lung Cancer/Pre-Malignant/GSE79209/GSE79209_collapsed_to_symbols.gct"
...
ggplot(plot_data, aes(x = Sex, y = Expression, color = Sex)) +
  ...
  labs(title = paste("GSE79209: Sex vs.", target_gene, "expression"),
       x = NULL,
...
ggplot(plot_data, aes(x = AgeGroup, y = Expression, color = AgeGroup)) +
  geom_jitter(width = 0.18, size = 2.5, alpha = 0.85, show.legend = FALSE) +
  stat_summary(fun = median, fun.min = median, fun.max = median,
               geom = "crossbar", width = 0.5, color = "#BEBEBE", size = 0.5, alpha = 0.5) +
  scale_color_manual(values = age_colors) +
  labs(title = paste("GSE79209: Age vs.", target_gene, "expression"),
       x = NULL,
       y = paste(target_gene, "expression (normalized)")) +
  theme_minimal(base_size = 15) +
  theme(
...
plot_data$Smoking.status <- factor(plot_data$Smoking.status,
                              levels = c("Current smoker", "Former smoker"),
                              ordered = TRUE)

# Define colors: blue for Male, red for Female
sex_colors <- c("Current smoker" = "#FB9A99", "Former smoker" = "#9ECAE1")

ggplot(plot_data, aes(x = Smoking.status, y = Expression, color = Smoking.status)) +
  geom_jitter(width = 0.15, size = 2.5, alpha = 0.85, show.legend = FALSE) +
  stat_summary(fun = median, fun.min = median, fun.max = median,
               geom = "crossbar", width = 0.5, color = "#BEBEBE", size = 0.5, alpha = 0.5) +
  scale_color_manual(values = sex_colors) +
  labs(title = paste("GSE79209: Smoking vs.", target_gene, "expression"),
       x = NULL,
       y = paste(target_gene, "expression (normalized)")) +
  theme_minimal(base_size = 15) +
  theme(
...
ggplot(plot_data, aes(x = Pack, y = Expression, color = Pack)) +
  geom_jitter(width = 0.18, size = 2.5, alpha = 0.85, show.legend = FALSE) +
  stat_summary(fun = median, fun.min = median, fun.max = median,
               geom = "crossbar", width = 0.5, color = "#BEBEBE", size = 0.5, alpha = 0.5) +
  scale_color_manual(values = pack_colors) +
  labs(title = paste("GSE79209: Pack-Years vs.", target_gene, "expression"),
       x = "Pack-Years",
       y = paste(target_gene, "expression (normalized)")) +
  theme_minimal(base_size = 15) +
  theme(
